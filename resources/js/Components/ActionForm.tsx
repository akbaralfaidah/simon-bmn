...
export default function ActionForm({ title, url, fields = [], initial = {}, label, description }: { title: string; url: string; fields?: Field[]; initial?: Record<string, any>; label?: string; description?: string }) {
    ...
    function submit(event: FormEvent) {
        ...
        form.post(url, { preserveScroll: true, onSuccess: () => { setOpen(false); form.reset(); } });
    }
    return <>
        <button type="button" className="simon-button" onClick={() => { form.clearErrors(); form.setData({ ...Object.fromEntries(fields.map(field => [field.name, ['multiselect', 'images'].includes(field.type || '') ? [] : field.type === 'file' ? null : ''])), ...initial }); setOpen(true); }}>{label || title}</button>
        <Modal show={open} onClose={() => !form.processing && setOpen(false)} maxWidth="lg" closeable={!form.processing}>
            <form onSubmit={submit} className="space-y-5 p-6">
                ...
                <div className="flex justify-end gap-3"><button className="simon-button-secondary" type="button" disabled={form.processing} onClick={() => setOpen(false)}>Batal</button><button className="simon-button" disabled={form.processing}>{form.processing ? 'Memproses…' : 'Konfirmasi'}</button></div>
            </form>
        </Modal>
    </>;
}
